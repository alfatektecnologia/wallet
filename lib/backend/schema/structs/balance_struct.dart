...
void addBalanceStructData(
  ...
  final clearFields =
      !forFieldValue && balanceStruct.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final balanceStructData =
      getBalanceFirestoreData(balanceStruct, forFieldValue);
  final nestedData =
      balanceStructData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = balanceStruct.firestoreUtilData.create || clearFields;
  firestoreData
...
List<Map<String, dynamic>> getBalanceListFirestoreData(
  List<BalanceStruct>? balanceStructs,
) =>
    balanceStructs?.map((e) => getBalanceFirestoreData(e, true)).toList() ?? [];
